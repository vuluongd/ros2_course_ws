...
- Record and replay data with the rosbag2
...
Terminal 1
   ...
   ┌───────┴────────┐
   ▼                ▼
tf_listener      tf2_echo
```

```
Terminal 1
dynamic_frame_publisher
           │
           ▼
          /tf
           │
...
```bash
#Terminal 1
ros2 run tf2_ros static_transform_publisher --x 0.15 --y 0 --z 0.20 --roll 0 --pitch 0 --yaw 0 --frame-id base_link --child-frame-id camera_link
# Terminal 2
ros2 run tf2_ros static_transform_publisher --x 0 --y 0 --z 0.25 --roll 0 --pitch 0 --yaw 0 --frame-id base_link --child-frame-id lidar_link
# Terminal 3
rviz2
```
...
| Classic Gazebo | Harmonic gazebo |
| ---------------- | ----------------- |
| `gazebo` command | `gz_sim` command |
| `libgazebo_ros_*.so` plugins | `gz-sim-*-system` plugins |
| Topic auto-bridged to ROS | Topic bridged via `ros_gz_bridge` |
...
Create `world/my_world.sdf` (SDF v1.10 for Gazebo Harmonic):
